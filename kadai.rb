...
printf('平均点：理科%d点, 英語%d点', rika_avg, eigo_avg) 
puts 

rika_arr = rika.map{|x| (x - rika_avg) ** 2}
eigo_arr = eigo.map{|x| (x - eigo_avg) ** 2}

rika_hyohen = Math.sqrt(rika_arr.sum / 20).round(2)
eigo_hyohen = Math.sqrt(eigo_arr.sum / 20).round(2)
...
rika_hensa = rika.map{|x| (10*(x-rika_avg)/rika_hyohen+50).round(2) }
